...
// ─── BAR CHART — memoized, only re-renders when data changes ──────────────────
const BarChart = memo(({ data, formatCurrency, isDark, labelKey, valueKey = 'sales' }) => {
  ...
  const maxVal = useMemo(() => Math.max(...data.map(d => d[valueKey] || 0), 1), [data, valueKey]);

  // Precompute labels once
  const labels = useMemo(() => data.map(item => {
    const raw = item[labelKey] || '';
    if (labelKey === 'date') return new Date(raw).toLocaleDateString('en-PH', { weekday: 'short' }).slice(0, 3);
    if (labelKey === 'month') return new Date(raw + '-01').toLocaleDateString('en-PH', { month: 'short' });
    if (labelKey === 'hour') { const h = parseInt(raw); return h < 12 ? `${h}am` : h === 12 ? '12pm' : `${h - 12}pm`; }
    return raw;
  }), [data, labelKey]);
        ...
        {data.map((item, i) => {
          ...
          return (
            <View key={i} style={{ flex: 1, alignItems: 'center', justifyContent: 'flex-end' }}>
              ...
              <View style={{ width: '100%', height: h, borderRadius: 6, backgroundColor: isMax ? '#F97316' : isDark ? '#3F3F46' : '#E5E7EB' }} />
            </View>
          );
        })}
      </View>
...
// ─── LINE CHART — memoized ────────────────────────────────────────────────────
const LineChart = memo(({ data, formatCurrency, isDark }) => {
  if (!data || data.length === 0) return (
    <View style={{ alignItems: 'center', paddingVertical: 32 }}>
  ...
  const { pts, maxVal, total } = useMemo(() => {
    const H = 100;
  ...
  return (
    <View>
      ...
      <View style={{ flexDirection: 'row', gap: 4, marginTop: 6, marginHorizontal: 4 }}>
        {pts.map((p, i) => (
          <Text key={i} style={{ flex: 1, fontSize: 9, textAlign: 'center', color: isDark ? '#71717A' : '#9CA3AF' }}>{p.label}</Text>
        ))}
      </View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: isDark ? '#27272A' : '#F3F4F6' }}>
        ...
        <View style={{ alignItems: 'center' }}>
          ...
          <Text style={{ fontSize: 16, fontWeight: '800', color: isDark ? '#FFFFFF' : '#111827' }}>{formatCurrency(maxVal)}</Text>
        </View>
      </View>
    </View>
  );
...
// ─── TOP ITEMS — memoized ─────────────────────────────────────────────────────
const TopItemsList = memo(({ items, formatCurrency, isDark }) => {
  if (!items || items.length === 0) return (
    <Text style={{ fontSize: 13, color: isDark ? '#52525B' : '#9CA3AF', textAlign: 'center', paddingVertical: 16 }}>No sales in this period</Text>
  );

  const maxSales = useMemo(() => Math.max(...items.map(i => i.total_sales || 0), 1), [items]);
  const medals = ['🥇', '🥈', '🥉'];
...
// ─── PAYMENT BREAKDOWN — memoized ─────────────────────────────────────────────
const PaymentBreakdown = memo(({ data, totalSales, formatCurrency, isDark }) => {
  if (!data || data.length === 0) return (
    <Text style={{ fontSize: 13, color: isDark ? '#52525B' : '#9CA3AF', textAlign: 'center', paddingVertical: 16 }}>No data for this period</Text>
  );
  ...
  const withPct = useMemo(() =>
    data.map(p => ({ ...p, pct: totalSales > 0 ? Math.round((p.total / totalSales) * 100) : 0 })),
    [data, totalSales]
  );
        ...
        <View key={p.payment_method}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              ...
              <Text style={{ fontSize: 11, color: '#F97316', fontWeight: '600' }}>{p.pct}%</Text>
            </View>
          </View>
          <View style={{ height: 5, backgroundColor: isDark ? '#3F3F46' : '#F3F4F6', borderRadius: 999 }}>
            <View style={{ height: 5, width: `${p.pct}%`, backgroundColor: '#F97316', borderRadius: 999, opacity: 0.8 }} />
          </View>
        </View>
      ))}
    </View>
...
export default function DashboardScreen() {
  ...
  // Stable period tab handlers — one per tab, never recreated
  const periodHandlers = useMemo(() =>
    PERIODS.reduce((acc, p) => { acc[p.id] = () => setPeriod(p.id); return acc; }, {}),
    []
  );
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: bg }} edges={['top']}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14, backgroundColor: surfBg, borderBottomWidth: 1, borderBottomColor: borderC }}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity onPress={() => setShowExport(true)} style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: isDark ? '#27272A' : '#F3F4F6', alignItems: 'center', justifyContent: 'center' }}>
              <Ionicons name="download-outline" size={20} color={isDark ? '#9CA3AF' : '#6B7280'} />
            </TouchableOpacity>
            <TouchableOpacity onPress={toggleTheme} style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: isDark ? '#27272A' : '#F3F4F6', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <View style={{ flexDirection: 'row', backgroundColor: surfBg, paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: borderC, gap: 8 }}>
          {PERIODS.map((p) => (
            <TouchableOpacity
              key={p.id}
              onPress={periodHandlers[p.id]}
              style={{ flex: 1, paddingVertical: 8, borderRadius: 12, alignItems: 'center', backgroundColor: period === p.id ? '#F97316' : isDark ? '#27272A' : '#F3F4F6' }}
            >
              <Text style={{ fontSize: 13, fontWeight: '700', color: period === p.id ? '#FFFFFF' : textMut }}>{p.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        >
          ...
          <Card>
            ...
            <BarChart data={chartData} formatCurrency={formatCurrency} isDark={isDark} labelKey={chartLabelKey} />
          </Card>
          ...
          <Card>
            ...
            <PaymentBreakdown data={summaryData?.byPayment} totalSales={totalSales} formatCurrency={formatCurrency} isDark={isDark} />
          </Card>
        </ScrollView>
      </SafeAreaView>

      {/* Conditionally mount — frees memory when closed */}
      {showExport && (
